...
export default function LeadForm({ isArabic, isTitle }) {
    ...
    const handleSubmit = async (event) => {
        ...
        if (isValid) {   
            ...
            await fetch(
                `https://test.salesforce.com/servlet/servlet.WebToLead?encoding=UTF-8&oid=${oid}&retURL=${retURL}&first_name=${firstName}&last_name=${lastName}&company=${company}&title=${title}&email=${email}&phone=${phone}&lead_source=${leadSource}`,
                {
                    method: 'POST',
        ...
        <form className={styles.form} >
            ...
            <div className={`${styles.inputContainer} ${inputContainerClass}`}>
                ...
                {/* <PhoneInput
                    ...
                    onChange={handlePhoneChange} // Use handlePhoneChange for phone input
                    dropdownStyle={{
                        top: '-200px', // Adjust this value as needed
                    }}
                /> */}
                {/* <input className={`${styles.inputAnimatedPhone} ${styles.inputAnimated}`}
                    type="tel"
                    id="phone"
                    name="phone"
                    value={formData.phone}
            ...
            <div className={styles.submitWrapper}>
                <button className={`${styles.submitBtn} ${submitBtnClass}`}
                    onClick={handleSubmit}>{isArabic ? 'إرسال' : 'SUBMIT'}</button>
            </div>
        </form>
    </>
    )
